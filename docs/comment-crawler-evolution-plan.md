...
| 14 | 已完成 | 抖音底部空转与登录误判修复 | 高 | 低 | 平台化登录检测、底部停机规则 | 抖音页面不再误报小红书登录墙，翻到底后可进入 idle |
...
抖音测试中发现两个问题：登录墙检测复用了小红书文案，导致抖音页面误报“请先登录小红书”；评论翻到底后，页面仍会持续产生少量候选文本，旧逻辑把这些少量新增当作有效进展，空转计数一直被清零。

第一版修正为：

- 小红书登录墙检测只在 `xiaohongshu.com` 域名下触发。
- 抖音页面即使出现通用“请登录、登录后查看”等文案，也不会提示小红书登录墙。
- 页面无点击、滚动位置不变、每轮只新增少量候选评论时，不再视为有效进展。
- 保留大批量新增评论作为有效进展，避免真实加载中的评论被过早截断。

验收标准：

- 抖音 URL 不再输出“小红书登录墙”提示。
- 抖音评论滚动到底后，连续空转可累积并以 `idle` 停止。
- 单测覆盖平台误判和底部少量新增候选两类回归。
