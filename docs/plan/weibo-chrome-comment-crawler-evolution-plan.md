...
| 双排序去重 | 现有 `row_key` 依赖 batch 与候选 ID，跨排序流不能保证去重 | 优先采集 DOM 稳定评论 ID；无 ID 时以受限复合指纹去重并强制标为 `partial` |
| QA | 目前只保存单一 `capture-state.json` 观察值 | 分别记录两个排序流、一级评论计数和回复展开状态 |

评论身份分为两种确定性模式，均只能由 Chrome 在受限 DOM 范围内产生，模型不得推测、补写或改写任何身份字段：

1. **首选 DOM-ID 模式**：从可见 DOM 属性、评论链接或页面嵌入的公开评论标识中确定性读取 `source_comment_id`、父评论 ID 和根评论 ID。该模式在两个排序流、回复展开和覆盖率均通过时，才可以判定为 `ok`。
2. **复合指纹模式**：若页面未暴露稳定评论 ID，采集公开作者 UID 的 `href`、规范化后的评论正文、页面显示时间，以及回复上下文和根评论上下文；由程序按固定字段顺序构建 `source_composite_fingerprint`。它只用于候选合并、断点恢复和重复抑制，任务始终为 `partial`，不得声称双排序“全量完成”。

不得用用户名、模型抽取结果或任意缺项的文本组合猜测身份；两种模式所需证据均不可读取时，保留已采集证据并以明确原因结束为 `partial`。
...
既有微博帖子 URL 清单
  ...
  -> 在同一评论面板内采集“按时间”流
       -> 精确展开安全的二级回复 -> 容器滚动 -> DOM 候选批次
  -> 优先以 DOM 稳定 ID、否则以受限复合指纹合并候选；每次滚动只作为断点证据
...
profile 使用已实现的 `identityMode`：`dom_id` 或 `composite`。两种模式都保留 `identityAttributes`；`dom_id` 时 `identityAttributes.comment` 必须非空，`composite` 时允许为空。`compositeIdentity` 在 `dom_id` 时可选、在 `composite` 时必填，且只能含 `authorHrefSelector`、`commentTextSelector`、`timestampSelector` 三个 selector。回复/根评论上下文不属于 profile selector，必须在既有已验证的回复/根 DOM scope 内推导。只有当评论面板和评论节点都能证明唯一性，才把 selector 写入 `PLATFORM_PROFILES.weibo`。探测失败不进入自动滚动阶段。
...
`capture_sort_mode` 只能为 `hot` 或 `time`；`identity_mode` 只能为 `dom_id` 或 `composite_fingerprint`。DOM-ID 模式的三个来源 ID 必须由 Chrome 读取 DOM 得到；没有值时保留空字符串并记录其来源缺失。复合指纹模式必须同时记录公开作者 UID `href`、`source_comment_text` 的规范化值、时间、回复上下文和根评论上下文，且 `source_composite_fingerprint` 仅由这些原始 DOM 证据确定性生成。候选去重优先级固定为：`source_comment_id`，其次为 `source_composite_fingerprint`；后者不得满足 `ok` 或双流全量门槛。
...
归一化器按 `source_chunk_id` 查回 candidate 元数据。DOM-ID 模式确定性写入 `comment_id`、`parent_comment_id`、`root_comment_id`，其 `row_key` 只基于 `platform + source_url + source_comment_id` 生成。复合指纹模式保持三类评论 ID 为空，其 `row_key` 基于 `platform + source_url + source_composite_fingerprint` 生成，并在原始证据中保留 `identity_mode`；同一评论在热度流、时间流或断点重跑中仍可合并，但不可升级为 `ok`。新增 `src/adapters/weibo.js` 从微博 canonical URL 生成稳定 `post_id`。
...
| 结果 | 判定 |
|---|---|
| `ok` | 热度流和时间流均完成；安全回复展开耗尽；所有候选均使用可复现的 DOM-ID 身份；所有模型批次、标准化和 QA 通过；一级评论计数与平台可见声明值达到既有 80% 质量阈值（如声明值可读） |
| `partial` | 任一排序不可安全切换、使用复合指纹身份、身份证据缺失、可见评论数量低于阈值、回复不能完全展开、模型批次缺失、滚动无进展或已知 UI 限制；复合指纹模式即使双流均结束也固定为 `partial` |
...
验收：每个自动化动作均有唯一 scope；至少一个热点流与时间流切换可验证；至少一条一级评论和一条二级回复均具有可复现的身份证据——优先稳定 DOM-ID，否则完整的公开 UID `href`、规范化正文、时间、回复上下文和根评论上下文。仅后者可继续实施，但探测记录必须标示 `composite_fingerprint` 与 `partial` 限制；两者均不满足时暂停自动化实现并输出受阻证据。

### 阶段 B：Chrome profile 与候选身份

实现 Weibo profile、唯一根验证、评论入口、排序流状态机、精确回复展开、容器滚动和扩展候选 schema。profile 用 `identityMode` 声明模式：`dom_id` 使用非空的 `identityAttributes.comment`；`composite` 要求 `compositeIdentity.authorHrefSelector`、`commentTextSelector`、`timestampSelector`，回复/根上下文在既有 scope 内推导。为无进展、错误排序状态、意外新标签、登录墙、身份证据缺失、二级回复上下文缺失和复合指纹降级写独立测试。

验收：模拟测试覆盖每一种拒绝路径；真实页面的热度流和时间流分别写出可验证的候选 batch 与状态。

### 阶段 C：模型结构化与标准化

实现微博 URL adapter、AI extraction schema 的 `weibo` 枚举、严格 schema 副本生成调用，以及基于 DOM-ID 或复合指纹的跨流去重与父子关系写回。

验收：同一 `source_comment_id` 或同一完整 `source_composite_fingerprint` 出现在两个 batch 后仅保留一条 normalized row；模型无法输出、篡改或补全 DOM-ID/复合指纹证据；严格模型 schema 能被本机 Codex CLI 接受。复合指纹样例 QA 必须为 `partial`。
...
| 文件 | 计划改动 |
|---|---|
| `src/browser/chrome-comment-capture.js` | 增加受实页探测验证的 Weibo profile、排序流、DOM-ID 与复合指纹候选证据支持 |
| `schemas/comment-dom-batch.schema.json` | 允许 `weibo` 及候选的排序、DOM-ID/复合指纹来源证据字段 |
| `schemas/ai-comment-extraction.schema.json` | 允许 `weibo`，保持模型不负责身份字段 |
| `src/adapters/weibo.js` | 解析和规范化微博帖子 URL 标识 |
| `src/normalize/normalize-ai-comment-extraction.js` | 回填 DOM-ID 或保留复合指纹证据、微博 `post_id`、跨流稳定去重 |
